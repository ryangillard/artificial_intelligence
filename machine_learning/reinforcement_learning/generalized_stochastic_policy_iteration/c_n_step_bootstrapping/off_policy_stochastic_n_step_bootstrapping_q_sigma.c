...
int main(int argc, char* argv[])
{
	...
	for (i = 0; i < (n_steps + 1); i++)
	{
		...
		episode_log[i].sigma = 0.0;
		episode_log[i].importance_sampling_ratio = 1.0;
	} // end of i loop
...
{
	int t, i, k, tau = 0;
	unsigned int t_mod_n_plus_1 = 0, t_plus_1_mod_n_plus_1 = 0, k_mod_n_plus_1 = 0, tau_mod_n_plus_1 = 0, successor_state_transition_index;
	...
	for (t = 0; t < maximum_episode_length; t++)
	{
		...
		if (t < max_timestep)
		{
			...
			else
			{
				...
				/* Get next sigma */
				episode_log[t_plus_1_mod_n_plus_1].sigma = (double)(rand() % 2);
				
				/* Get next importance sampling ratio */
				episode_log[t_plus_1_mod_n_plus_1].importance_sampling_ratio = target_policy[episode_log[t_plus_1_mod_n_plus_1].state_index][episode_log[t_plus_1_mod_n_plus_1].action_index] / behavior_policy[episode_log[t_plus_1_mod_n_plus_1].state_index][episode_log[t_plus_1_mod_n_plus_1].action_index];
			}
		}
		...
		if (tau >= 0)
		{
			...
			for (k = min(t + 1, max_timestep); k >= tau + 1; k--)
			{
				if (k == max_timestep)
				{
					/* Calculate expected return */
				...
				else
				{
					...
					{
						state_value_function_expected_value_on_policy += target_policy[episode_log[k_mod_n_plus_1].state_index][i] * state_action_value_function[episode_log[k_mod_n_plus_1].state_index][i];
					} // end of j loop
					
					/* Calculate expected return */
					expected_return = episode_log[k_mod_n_plus_1].reward + discounting_factor_gamma * ((episode_log[k_mod_n_plus_1].sigma * episode_log[k_mod_n_plus_1].importance_sampling_ratio + (1.0 - episode_log[k_mod_n_plus_1].sigma) * target_policy[episode_log[k_mod_n_plus_1].state_index][episode_log[k_mod_n_plus_1].action_index]) * (expected_return - state_action_value_function[episode_log[k_mod_n_plus_1].state_index][episode_log[k_mod_n_plus_1].action_index]) + state_value_function_expected_value_on_policy);
				}
			} // end of k loop
