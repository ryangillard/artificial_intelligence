	...
	for (t = 0; t < maximum_episode_length; t++)
	{
		...
		if (tau >= 0)
		{
			...
			if (tau + n_steps < max_timestep)
			{
				...
				{
					state_value_function_expected_value_on_policy += policy[episode_log[tau_plus_n_mod_n_plus_1].state_index][i] * state_action_value_function[episode_log[tau_plus_n_mod_n_plus_1].state_index][i];
				} // end of i loop
			...
			EpsilonGreedyPolicyFromStateActionFunction(number_of_actions_per_non_terminal_state, state_action_value_function, epsilon, episode_log[tau_mod_n_plus_1].state_index, policy, policy_cumulative_sum);
		}
