...
int main(int argc, char* argv[])
{
	...
	for (i = 0; i < number_of_non_terminal_states; i++)
	{
		...
		for (j = 0; j < number_of_actions_per_non_terminal_state[i]; j++)
		{
			target_policy[i][j] = 1.0 / number_of_actions_per_non_terminal_state[i];
		} // end of j loop
	} // end of i loop
	
	/* Create target policy cumulative sum array */
	double** target_policy_cumulative_sum;
	target_policy_cumulative_sum = malloc(sizeof(double*) * number_of_non_terminal_states);
	for (i = 0; i < number_of_non_terminal_states; i++)
	{
		target_policy_cumulative_sum[i] = malloc(sizeof(double) * number_of_actions_per_non_terminal_state[i]);
		target_policy_cumulative_sum[i][0] = target_policy[i][0];
		for (j = 1; j < number_of_actions_per_non_terminal_state[i]; j++)
		{
			target_policy_cumulative_sum[i][j] = target_policy_cumulative_sum[i][j - 1] + target_policy[i][j];
		} // end of j loop
	} // end of i loop
	...
	for (i = 0; i < number_of_non_terminal_states; i++)
	{
		...
		for (j = 0; j < number_of_actions_per_non_terminal_state[i]; j++)
		{
			behavior_policy[i][j] = 1.0 / number_of_actions_per_non_terminal_state[i];
		} // end of j loop
	} // end of i loop
	...
	for (i = 0; i < number_of_episodes; i++)
	{
		...
		max_timestep = InitializeEpisode(number_of_non_terminal_states, number_of_actions_per_non_terminal_state, state_action_value_function, maximum_episode_length, behavior_policy_cumulative_sum, episode_log);

		/* Loop through episode and update the policy */
		LoopThroughEpisode(number_of_non_terminal_states, number_of_actions_per_non_terminal_state, number_of_state_action_successor_states, state_action_successor_state_indices, state_action_successor_state_transition_probabilities_cumulative_sum, state_action_successor_state_rewards, state_action_value_function, target_policy, target_policy_cumulative_sum, behavior_policy, behavior_policy_cumulative_sum, alpha, discounting_factor_gamma, maximum_episode_length, max_timestep, episode_log, n_steps);
	} // end of i loop
...
{
	int t, i, tau = 0;
	unsigned int t_mod_n_plus_1, t_plus_1_mod_n_plus_1, i_mod_n_plus_1, tau_plus_n_mod_n_plus_1, tau_mod_n_plus_1, successor_state_transition_index;
	double probability, importance_sampling_ratio = 1.0, state_value_function_expected_value_on_policy = 0.0, expected_return = 0.0;
	...
	for (t = 0; t < maximum_episode_length; t++)
	{
		...
		if (t < max_timestep)
		{
			...
			/* Check to see if we actioned into a terminal state OR if we hit the max episode length */
			...
			else
			{
				...
				{
					if (probability <= behavior_policy_cumulative_sum[episode_log[t_plus_1_mod_n_plus_1].state_index][i])
					{
		...
		if (tau >= 0)
		{
			...
			for (i = tau + 1; i <= min(tau + n_steps - 1, max_timestep - 1); i++)
			{
				/* Spend a little memory to save computation time */
				i_mod_n_plus_1 = i % (n_steps + 1);
				
				importance_sampling_ratio *= target_policy[episode_log[i_mod_n_plus_1].state_index][episode_log[i_mod_n_plus_1].action_index] / behavior_policy[episode_log[i_mod_n_plus_1].state_index][episode_log[i_mod_n_plus_1].action_index];
			} // end of i loop
			...
			for (i = tau + 1; i <= min(tau + n_steps, max_timestep); i++)
			{
				...
				expected_return += pow(discounting_factor_gamma, i - tau - 1) * episode_log[i_mod_n_plus_1].reward;
			} // end of j loop
			...
			if (tau + n_steps < max_timestep)
			{
				...
				for (i = 0; i < number_of_actions_per_non_terminal_state[episode_log[tau_plus_n_mod_n_plus_1].state_index]; i++)
				{
					state_value_function_expected_value_on_policy += target_policy[episode_log[tau_plus_n_mod_n_plus_1].state_index][i] * state_action_value_function[episode_log[tau_plus_n_mod_n_plus_1].state_index][i];
				} // end of i loop
			...
			state_action_value_function[episode_log[tau_mod_n_plus_1].state_index][episode_log[tau_mod_n_plus_1].action_index] += alpha * importance_sampling_ratio * (expected_return - state_action_value_function[episode_log[tau_mod_n_plus_1].state_index][episode_log[tau_mod_n_plus_1].action_index]);
			
			/* Choose policy for chosen state by greedily choosing from the state-action-value function */
			GreedyPolicyFromStateActionFunction(number_of_actions_per_non_terminal_state, state_action_value_function, episode_log[tau_mod_n_plus_1].state_index, target_policy, target_policy_cumulative_sum);
		}
