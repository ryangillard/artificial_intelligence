...
int main(int argc, char* argv[])
{
	...
	for (i = 0; i < number_of_episodes; i++)
	{
		...
		InitializeEpisode(number_of_non_terminal_states, number_of_actions_per_non_terminal_state, state_action_value_function1, state_action_value_function2, epsilon, policy, policy_cumulative_sum, &initial_state_index, &initial_action_index);
...
void InitializeEpisode(unsigned int number_of_non_terminal_states, unsigned int* number_of_actions_per_non_terminal_state, double** state_action_value_function1, double** state_action_value_function2, double epsilon, double** policy, double** policy_cumulative_sum, unsigned int* initial_state_index, unsigned int* initial_action_index)
{
	...
	EpsilonGreedyPolicyFromStateActionFunction(number_of_actions_per_non_terminal_state, state_action_value_function1, state_action_value_function2, epsilon, (*initial_state_index), policy, policy_cumulative_sum);
...
{
	unsigned int i, j;
	unsigned int successor_state_transition_index, next_state_index;
	...
	for (i = 0; i < maximum_episode_length; i++)
	{
		...
		if (probability <= 0.5)
		{
			UpdateStateActionValueFunction(number_of_non_terminal_states, number_of_actions_per_non_terminal_state, state_action_value_function2, policy, policy_cumulative_sum, alpha, epsilon, discounting_factor_gamma, &state_index, &action_index, reward, next_state_index, state_action_value_function1);
		}
		else
		{
			UpdateStateActionValueFunction(number_of_non_terminal_states, number_of_actions_per_non_terminal_state, state_action_value_function1, policy, policy_cumulative_sum, alpha, epsilon, discounting_factor_gamma, &state_index, &action_index, reward, next_state_index, state_action_value_function2);
		}
...
{
	unsigned int i;
	unsigned int next_action_index;
	double probability;
	...
	else
	{
		...
		/* Choose policy for chosen state by epsilon-greedy choosing from the state-action-value function */
		EpsilonGreedyPolicyFromStateActionFunction(number_of_actions_per_non_terminal_state, updating_state_action_value_function, not_updating_state_action_value_function, epsilon, next_state_index, policy, policy_cumulative_sum);
