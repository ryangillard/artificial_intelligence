...
void LoopThroughEpisode(unsigned int number_of_non_terminal_states, unsigned int* number_of_actions_per_non_terminal_state, unsigned int** number_of_state_action_successor_states, unsigned int*** state_action_successor_state_indices, double*** state_action_successor_state_transition_probabilities_cumulative_sum, double*** state_action_successor_state_rewards, double** state_action_value_function1, double** state_action_value_function2, double** policy, double** policy_cumulative_sum, double alpha, double epsilon, double discounting_factor_gamma, unsigned int maximum_episode_length, unsigned int state_index)
{
	...
	for (i = 0; i < maximum_episode_length; i++)
	{
		...
		if (probability <= 0.5)
		{
			UpdateStateActionValueFunction(number_of_non_terminal_states, number_of_actions_per_non_terminal_state, state_action_value_function2, policy, alpha, epsilon, discounting_factor_gamma, state_index, action_index, reward, next_state_index, state_action_value_function1);
		}
		else
		{
			UpdateStateActionValueFunction(number_of_non_terminal_states, number_of_actions_per_non_terminal_state, state_action_value_function1, policy, alpha, epsilon, discounting_factor_gamma, state_index, action_index, reward, next_state_index, state_action_value_function2);
		}
...
unsigned int UpdateStateActionValueFunction(unsigned int number_of_non_terminal_states, unsigned int* number_of_actions_per_non_terminal_state, double** not_updating_state_action_value_function, double** policy, double alpha, double epsilon, double discounting_factor_gamma, unsigned int state_index, unsigned int action_index, double reward, unsigned int next_state_index, double** updating_state_action_value_function)
{
	...
	else
	{
		...
		{
			not_updating_state_value_function_expected_value_on_policy += policy[next_state_index][i] * not_updating_state_action_value_function[next_state_index][i];
		} // end of i loop
