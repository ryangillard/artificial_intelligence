...
int main(int argc, char* argv[])
{
	...
	unsigned int number_of_episodes = 150000;
	
	/* Set the maximum episode length */
	unsigned int maximum_episode_length = 200;
	
	/* Set the number of state-action-value functions */
	unsigned int number_of_state_action_value_functions = 3;
	
	/* Create state-action-value function arrays */
	double*** state_action_value_functions;
	state_action_value_functions = malloc(sizeof(double**) * number_of_state_action_value_functions);
	for (i = 0; i < number_of_state_action_value_functions; i++)
	{
		state_action_value_functions[i] = malloc(sizeof(double*) * number_of_states);
		for (j = 0; j < number_of_states; j++)
		{
			state_action_value_functions[i][j] = malloc(sizeof(double) * number_of_actions_per_state[j]);
			for (k = 0; k < number_of_actions_per_state[j]; k++)
			{
				state_action_value_functions[i][j][k] = 0.0;
			} // end of k loop
		} // end of j loop
	} // end of i loop
	...
	{
		printf("Q_%u\n", i);
		for (j = 0; j < number_of_non_terminal_states; j++)
		{
			printf("%u", j);
			for (k = 0; k < number_of_actions_per_non_terminal_state[j]; k++)
			{
				printf("\t%lf", state_action_value_functions[i][j][k]);
			} // end of k loop
			printf("\n");
		} // end of j loop
		printf("\n");
	...
	for (i = 0; i < number_of_episodes; i++)
	{
		...
		InitializeEpisode(number_of_non_terminal_states, number_of_actions_per_non_terminal_state, number_of_state_action_value_functions, state_action_value_functions, epsilon, policy, policy_cumulative_sum, &initial_state_index, &initial_action_index);
	...
	for (i = 0; i < number_of_state_action_value_functions; i++)
	{
		for (j = 0; j < number_of_states; j++)
		{
			free(state_action_value_functions[i][j]);	
		} // end of j loop
		free(state_action_value_functions[i]);
	} // end of i loop
	free(state_action_value_functions);
...
void InitializeEpisode(unsigned int number_of_non_terminal_states, unsigned int* number_of_actions_per_non_terminal_state, unsigned int number_of_state_action_value_functions, double*** state_action_value_functions, double epsilon, double** policy, double** policy_cumulative_sum, unsigned int* initial_state_index, unsigned int* initial_action_index)
{
	...
	EpsilonGreedyPolicyFromStateActionFunction(number_of_actions_per_non_terminal_state, number_of_state_action_value_functions, state_action_value_functions, epsilon, (*initial_state_index), policy, policy_cumulative_sum);
...
void EpsilonGreedyPolicyFromStateActionFunction(unsigned int* number_of_actions_per_non_terminal_state, unsigned int number_of_state_action_value_functions, double*** state_action_value_functions, double epsilon, unsigned int state_index, double** policy, double** policy_cumulative_sum)
{
	unsigned int i, j, max_action_count = 1;
	double state_action_value = 0.0, max_state_action_value = -DBL_MAX, max_policy_apportioned_probability_per_action = 1.0, remaining_apportioned_probability_per_action = 0.0;
	...
	for (i = 0; i < number_of_actions_per_non_terminal_state[state_index]; i++)
	{
		...
		for (j = 0; j < number_of_state_action_value_functions; j++)
		{
			state_action_value += state_action_value_functions[j][state_index][i];
		} // end of j loop
		...
		else if (state_action_value == max_state_action_value)
		{
			max_action_count++;
		}
	} // end of i loop
...
{
	unsigned int i, j;
	unsigned int successor_state_transition_index, next_state_index, updating_state_action_value_function_index;
	...
	for (i = 0; i < maximum_episode_length; i++)
	{
		...
		updating_state_action_value_function_index = rand() % number_of_state_action_value_functions;
		
		UpdateStateActionValueFunction(number_of_non_terminal_states, number_of_actions_per_non_terminal_state, policy, policy_cumulative_sum, alpha, epsilon, discounting_factor_gamma, &state_index, &action_index, reward, next_state_index, updating_state_action_value_function_index, number_of_state_action_value_functions, state_action_value_functions);
...
{
	unsigned int i;
	unsigned int next_action_index, not_updating_state_action_value_function_index = 0;
	double probability;
	
	/* Check to see if we actioned into a terminal state */
	if (next_state_index >= number_of_non_terminal_states)
	{
		state_action_value_functions[updating_state_action_value_function_index][(*state_index)][(*action_index)] += alpha * (reward - state_action_value_functions[updating_state_action_value_function_index][(*state_index)][(*action_index)]);
	}
	else
	{
		/* Get next action */
		...
		/* Choose policy for chosen state by epsilon-greedy choosing from the state-action-value function */
		EpsilonGreedyPolicyFromStateActionFunction(number_of_actions_per_non_terminal_state, number_of_state_action_value_functions, state_action_value_functions, epsilon, next_state_index, policy, policy_cumulative_sum);
		...
		if (number_of_state_action_value_functions > 1)
		{
			do
			{
				not_updating_state_action_value_function_index = rand() % number_of_state_action_value_functions;
			} while (not_updating_state_action_value_function_index == updating_state_action_value_function_index);
		}
