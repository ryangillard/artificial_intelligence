...
struct PredecessorStateActionPair
{
...
void UpdateModelOfEnvironmentFromExperience(unsigned int state_index, unsigned int action_index, double reward, unsigned int next_state_index, unsigned int number_of_states, unsigned int number_of_non_terminal_states, unsigned int* number_of_actions_per_non_terminal_state, unsigned int** model_number_of_state_action_successor_states, unsigned int**** model_state_action_successor_state_indices, double**** model_state_action_successor_state_transition_probabilities, double**** model_state_action_successor_state_transition_probabilities_cumulative_sum, double**** model_state_action_successor_state_rewards, unsigned int**** model_state_action_successor_state_number_of_visits, unsigned int* model_number_of_state_predecessor_state_action_pairs, struct PredecessorStateActionPair*** model_state_predecessor_state_action_pairs);
...
void Realloc1dStatePredecessorStateActionArray(unsigned int model_number_of_state_predecessor_state_action_pairs, struct PredecessorStateActionPair** model_state_predecessor_state_action_pairs);
...
void SearchAndUpdatePriorityQueue(unsigned int state_index, unsigned int action_index, double priority, unsigned int* current_priority_queue_size, struct PriorityQueueNode* priority_queue);

/* This function searches for a node in the priority queue */
int SearchPriorityQueue(unsigned int state_index, unsigned int action_index, unsigned int current_priority_queue_size, struct PriorityQueueNode* priority_queue);
...
void PriorityQueueNodeIncreasePriority(unsigned int priority_queue_index, double new_priority, struct PriorityQueueNode* priority_queue);

/* This function inserts a node into the priority queue */
void InsertIntoPriorityQueue(unsigned int state_index, unsigned int action_index, double priority, unsigned int* current_priority_queue_size, struct PriorityQueueNode* priority_queue);

/* This function pops max node off from priority queue */
struct PriorityQueueNode PopMaxNodeFromPriorityQueue(unsigned int* current_priority_queue_size, struct PriorityQueueNode* priority_queue);
...
/* This function swaps two priority queue nodes */
void SwapPriorityQueueNodes(struct PriorityQueueNode* x, struct PriorityQueueNode* y);

/* This function gets the parent index of the given priority queue node's index */
unsigned int ParentPriorityQueueNodeIndex(unsigned int priority_queue_index);

/* This function gets the left child index of the given priority queue node's index */
unsigned int LeftPriorityQueueNodeIndex(unsigned int priority_queue_index);

/* This function gets the right child index of the given priority queue node's index */
unsigned int RightPriorityQueueNodeIndex(unsigned int priority_queue_index);

/* This function reallocates more memory to the passed 1d priority queue node array */
void Realloc1dPriorityQueue(unsigned int max_priority_queue_size, struct PriorityQueueNode** priority_queue);
...
int main(int argc, char* argv[])
{
	...
	/* Track the number of predecessor state-action pairs of states */
	unsigned int* model_number_of_state_predecessor_state_action_pairs;
	
	model_number_of_state_predecessor_state_action_pairs = malloc(sizeof(unsigned int) * number_of_states);
	for (i = 0; i < number_of_states; i++)
	{
		model_number_of_state_predecessor_state_action_pairs[i] = 0;
	} // end of i loop
	
	/* Track the predecessor state-action pairs of states */
	struct PredecessorStateActionPair** model_state_predecessor_state_action_pairs;
	
	model_state_predecessor_state_action_pairs = malloc(sizeof(struct PredecessorStateActionPair*) * number_of_states);
	for (i = 0; i < number_of_states; i++)
	{
		model_state_predecessor_state_action_pairs[i] = malloc(sizeof(struct PredecessorStateActionPair) * 1);
		for (j = 0; j < 1; j++)
		{
			model_state_predecessor_state_action_pairs[i][j].state_index = 0;
			model_state_predecessor_state_action_pairs[i][j].action_index = 0;
		} // end of j loop
	} // end of i loop
	...
	unsigned int number_of_planning_steps = 5;
	...
	unsigned int current_priority_queue_size = 0, max_priority_queue_size = 0;
	
	for (i = 0; i < number_of_non_terminal_states; i++)
	{
		max_priority_queue_size += number_of_actions_per_non_terminal_state[i];
	} // end of i loop
	
	struct PriorityQueueNode* priority_queue;
	priority_queue = malloc(sizeof(struct PriorityQueueNode) * max_priority_queue_size);
	for (i = 0; i < max_priority_queue_size; i++)
	{
		priority_queue[i].state_index = i;
		priority_queue[i].action_index = i * i;
		priority_queue[i].priority = -DBL_MAX;
	}
	priority_queue[0].priority = DBL_MAX;
	...
	for (i = 0; i < number_of_states; i++)
	{
		free(model_state_predecessor_state_action_pairs[i]);
	} // end of i loop
	free(model_state_predecessor_state_action_pairs);
	free(model_number_of_state_predecessor_state_action_pairs);
...
void UpdateModelOfEnvironmentFromExperience(unsigned int state_index, unsigned int action_index, double reward, unsigned int next_state_index, unsigned int number_of_states, unsigned int number_of_non_terminal_states, unsigned int* number_of_actions_per_non_terminal_state, unsigned int** model_number_of_state_action_successor_states, unsigned int**** model_state_action_successor_state_indices, double**** model_state_action_successor_state_transition_probabilities, double**** model_state_action_successor_state_transition_probabilities_cumulative_sum, double**** model_state_action_successor_state_rewards, unsigned int**** model_state_action_successor_state_number_of_visits, unsigned int* model_number_of_state_predecessor_state_action_pairs, struct PredecessorStateActionPair*** model_state_predecessor_state_action_pairs)
{
	...
	for (i = 0; i < model_number_of_state_predecessor_state_action_pairs[next_state_index]; i++)
	{
		if ((*model_state_predecessor_state_action_pairs)[next_state_index][i].state_index == state_index && (*model_state_predecessor_state_action_pairs)[next_state_index][i].action_index == action_index)
		{
			found_predecessor = 1;
			break; // break i loop, found predecessor so no need to search more
		}
	} // end of i loop
	
	if (found_predecessor == 0)
	{
		if (model_number_of_state_predecessor_state_action_pairs[next_state_index] > 0)
		{
			/* Realloc state predecessor state-action array */
			Realloc1dStatePredecessorStateActionArray(model_number_of_state_predecessor_state_action_pairs[next_state_index], &(*model_state_predecessor_state_action_pairs)[next_state_index]);
		}
		
		(*model_state_predecessor_state_action_pairs)[next_state_index][model_number_of_state_predecessor_state_action_pairs[next_state_index]].state_index = state_index;
		(*model_state_predecessor_state_action_pairs)[next_state_index][model_number_of_state_predecessor_state_action_pairs[next_state_index]].action_index = action_index;
		
		model_number_of_state_predecessor_state_action_pairs[next_state_index]++;
	}
...
{
	unsigned int i;
	struct PredecessorStateActionPair* temp = NULL;

	/* Malloc first dimension of temp array (row) */
	temp = malloc(sizeof(struct PredecessorStateActionPair) * model_number_of_state_predecessor_state_action_pairs);

	/* Transfer 1d array's elements to old rectanglular prism of temp array */
	for (i = 0; i < model_number_of_state_predecessor_state_action_pairs; i++)
	{
		temp[i] = (*model_state_predecessor_state_action_pairs)[i];
	} // end of i loop

	/* Free 1d array */
	free(*model_state_predecessor_state_action_pairs);

	/* Malloc 1d array's first dimension (rows) */
	(*model_state_predecessor_state_action_pairs) = malloc(sizeof(struct PredecessorStateActionPair) * (model_number_of_state_predecessor_state_action_pairs + 1));

	/* Transfer temp array's elements from the old rectangular prism back to 1d array */
	for (i = 0; i < model_number_of_state_predecessor_state_action_pairs; i++)
	{
		(*model_state_predecessor_state_action_pairs)[i] = temp[i];
	} // end of i loop
	...
	return;
} // end of Realloc1dStatePredecessorStateActionArray function
...
void ModelSimualatePlanning(unsigned int number_of_planning_steps, unsigned int number_of_non_terminal_states, unsigned int* number_of_actions_per_non_terminal_state, unsigned int model_number_of_seen_non_terminal_states, unsigned int* model_seen_non_terminal_states_stack, unsigned int* model_seen_non_terminal_states_stack_reverse_lookup, unsigned int* model_number_of_seen_non_terminal_states_actions, unsigned int** model_seen_non_terminal_states_actions_stack, unsigned int** model_seen_non_terminal_states_actions_stack_reverse_lookup, unsigned int** model_number_of_state_action_successor_states, unsigned int*** model_state_action_successor_state_indices, double*** model_state_action_successor_state_transition_probabilities, double*** model_state_action_successor_state_transition_probabilities_cumulative_sum, double*** model_state_action_successor_state_rewards, unsigned int*** model_state_action_successor_state_number_of_visits, unsigned int* model_number_of_state_predecessor_state_action_pairs, struct PredecessorStateActionPair** model_state_predecessor_state_action_pairs, unsigned int** state_action_value_function_max_tie_stack, double alpha, double discounting_factor_gamma, double theta, double** state_action_value_function, unsigned int* current_priority_queue_size, unsigned int* max_priority_queue_size, struct PriorityQueueNode** priority_queue)
{
	...
	for (i = 0; i < number_of_planning_steps; i++)
	{
		...
		max_priority_state_action_pair = PopMaxNodeFromPriorityQueue(current_priority_queue_size, (*priority_queue));
		
		state_index = max_priority_state_action_pair.state_index;
		...
		for (j = 0; j < model_number_of_state_predecessor_state_action_pairs[state_index]; j++)
		{
			predecessor_state_index = model_state_predecessor_state_action_pairs[state_index][j].state_index;
			predecessor_action_index = model_state_predecessor_state_action_pairs[state_index][j].action_index;
			
			/* Get reward */
			for (k = 0; k < model_number_of_state_action_successor_states[predecessor_state_index][predecessor_action_index]; k++)
			{
				if (model_state_action_successor_state_indices[predecessor_state_index][predecessor_action_index][k] == state_index)
				{
					successor_state_transition_index = k;
					break; // break k loop since we found our index
				}
			} // end of k loop
			...
			if (priority > theta)
			{
				...
				SearchAndUpdatePriorityQueue(predecessor_state_index, predecessor_action_index, priority, current_priority_queue_size, (*priority_queue));
			}
		} // end of j loop
	} // end of i loop
...
void SearchAndUpdatePriorityQueue(unsigned int state_index, unsigned int action_index, double priority, unsigned int* current_priority_queue_size, struct PriorityQueueNode* priority_queue)
{
	int priority_queue_index = -1;
	priority_queue_index = SearchPriorityQueue(state_index, action_index, (*current_priority_queue_size), priority_queue);
	...
	if (priority_queue_index >= 0)
	{
		...
		{
			PriorityQueueNodeIncreasePriority(priority_queue_index, priority, priority_queue);
		}
	}
	else
	{
		/* Node wasn't found so insert into priority queue */
		InsertIntoPriorityQueue(state_index, action_index, priority, current_priority_queue_size, priority_queue);
	}

	return;
} // end of SearchAndUpdatePriorityQueue function
...
int SearchPriorityQueue(unsigned int state_index, unsigned int action_index, unsigned int current_priority_queue_size, struct PriorityQueueNode* priority_queue)
{
	...
	for (i = 0; i < current_priority_queue_size; i++)
	{
		if (priority_queue[i].state_index == state_index && priority_queue[i].action_index == action_index)
		{
...
} // end of SearchPriorityQueue function

/* This function increases priority at priority_queue_index to new_priority, where it is assumed that new_priority is greater than priority_queue[priority_queue_index] */
void PriorityQueueNodeIncreasePriority(unsigned int priority_queue_index, double new_priority, struct PriorityQueueNode* priority_queue)
{
	priority_queue[priority_queue_index].priority = new_priority;
	while (priority_queue_index != 0 && priority_queue[ParentPriorityQueueNodeIndex(priority_queue_index)].priority < priority_queue[priority_queue_index].priority)
	{
		SwapPriorityQueueNodes(&priority_queue[priority_queue_index], &priority_queue[ParentPriorityQueueNodeIndex(priority_queue_index)]);
		priority_queue_index = ParentPriorityQueueNodeIndex(priority_queue_index);
	}
	
	return;
} // end of PriorityQueueNodeIncreasePriority function
...
void InsertIntoPriorityQueue(unsigned int state_index, unsigned int action_index, double priority, unsigned int* current_priority_queue_size, struct PriorityQueueNode* priority_queue)
{
	...
	unsigned int priority_queue_index = (*current_priority_queue_size) - 1;
	...
	return;
} // end of InsertIntoPriorityQueue function
...
struct PriorityQueueNode PopMaxNodeFromPriorityQueue(unsigned int* current_priority_queue_size, struct PriorityQueueNode* priority_queue)
{
	...
	struct PriorityQueueNode root = priority_queue[0];
	priority_queue[0] = priority_queue[(*current_priority_queue_size) - 1];
	(*current_priority_queue_size)--;
...
void MaxHeapifyPriorityQueue(unsigned int priority_queue_index, unsigned int current_priority_queue_size, struct PriorityQueueNode* priority_queue)
{
	unsigned int l = LeftPriorityQueueNodeIndex(priority_queue_index);
	unsigned int r = RightPriorityQueueNodeIndex(priority_queue_index);
	unsigned int biggest = priority_queue_index;
	
	if (l < current_priority_queue_size && priority_queue[l].priority > priority_queue[priority_queue_index].priority)
	{
		biggest = l;
	}
	
	if (r < current_priority_queue_size && priority_queue[r].priority > priority_queue[biggest].priority)
	{
		biggest = r;
	}
	...
	{
		SwapPriorityQueueNodes(&priority_queue[priority_queue_index], &priority_queue[biggest]);
		
		MaxHeapifyPriorityQueue(biggest, current_priority_queue_size, priority_queue);
	}
	
	return;
} // end of MaxHeapifyPriorityQueue function

/* This function swaps two priority queue nodes */
void SwapPriorityQueueNodes(struct PriorityQueueNode* x, struct PriorityQueueNode* y)
{
	struct PriorityQueueNode temp = *x;
	*x = *y;
	*y = temp;
	
	return;
} // end of SwapPriorityQueueNodes function

/* This function gets the parent index of the given priority queue node's index */
unsigned int ParentPriorityQueueNodeIndex(unsigned int priority_queue_index)
{
	return (priority_queue_index - 1) / 2;
} // end of ParentPriorityQueueNodeIndex function
...
} // end of RightPriorityQueueNodeIndex function

/* This function reallocates more memory to the passed 1d priority queue node array */
void Realloc1dPriorityQueue(unsigned int max_priority_queue_size, struct PriorityQueueNode** priority_queue)
{
	unsigned int i;
	struct PriorityQueueNode* temp = NULL;

	/* Malloc first dimension of temp array (row) */
	temp = malloc(sizeof(struct PriorityQueueNode) * max_priority_queue_size);
	...
	/* Malloc 1d array's first dimension (rows) */
	(*priority_queue) = malloc(sizeof(struct PriorityQueueNode) * (max_priority_queue_size * 2));

	/* Transfer temp array's elements from the old rectangular prism back to 1d array */
	for (i = 0; i < max_priority_queue_size; i++)
	{
		(*priority_queue)[i] = temp[i];
	} // end of i loop
	...
	return;
} // end of Realloc1dPriorityQueue function
