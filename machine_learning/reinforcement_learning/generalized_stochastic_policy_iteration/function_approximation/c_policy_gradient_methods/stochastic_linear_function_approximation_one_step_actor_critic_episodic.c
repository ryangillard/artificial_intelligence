...
unsigned int InitializeEpisode(unsigned int number_of_non_terminal_states, double* gamma_t);
	...
	for (t = 0; t < maximum_episode_length; t++)
	{
		...
		for (i = 0; i < max_number_of_actions; i++)
		{
			if (i == action_index)
			{
				...
				{
					policy_weights[i * number_of_features + j] += policy_weight_alpha * gamma_t * delta * ((1.0 - policy[i]) * feature_vector[j]);
				} // end of j loop
			}
			else
			{
				for (j = 0; j < number_of_features; j++)
				{
					policy_weights[i * number_of_features + j] += policy_weight_alpha * gamma_t * delta * (-policy[i] * feature_vector[j]);
				} // end of j loop				
			}
		} // end of i loop
