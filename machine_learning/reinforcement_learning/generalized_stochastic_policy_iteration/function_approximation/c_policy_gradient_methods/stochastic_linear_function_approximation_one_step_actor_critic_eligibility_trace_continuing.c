...
unsigned int InitializeEpisode(unsigned int number_of_states, unsigned int number_of_policy_weights, double* policy_eligibility_trace, unsigned int number_of_features, double* state_value_eligibility_trace);
...
int main(int argc, char* argv[])
{
	...
	/* Set state-value weight learning rate alpha */
	double average_reward_estimate_alpha = 0.001;
...
unsigned int InitializeEpisode(unsigned int number_of_states, unsigned int number_of_policy_weights, double* policy_eligibility_trace, unsigned int number_of_features, double* state_value_eligibility_trace)
{
	...
	/* Initial state */
	state_index = rand() % number_of_states; // randomly choose an initial state from all states
...
void LoopForever(unsigned int number_of_states, unsigned int** number_of_state_action_successor_states, unsigned int*** state_action_successor_state_indices, double*** state_action_successor_state_transition_probabilities_cumulative_sum, double*** state_action_successor_state_rewards, unsigned int max_number_of_actions, unsigned int number_of_state_tilings, unsigned int number_of_state_tiles, double** state_double_variables, unsigned int number_of_state_double_variables, int** state_int_variables, unsigned int number_of_state_int_variables, unsigned int* state_tile_indices, unsigned int number_of_features, double* feature_vector, double* next_feature_vector, unsigned int number_of_policy_weights, double* policy_weights, double* state_value_weights, double* policy, double* policy_cumulative_sum, double* policy_eligibility_trace, double* state_value_eligibility_trace, double policy_weight_alpha, double state_value_weight_alpha, double average_reward_estimate_alpha, double average_reward_estimate, double policy_trace_decay_lambda, double state_value_trace_decay_lambda, unsigned int maximum_episode_length, unsigned int state_index)
{
	...
	for (t = 0; t < maximum_episode_length; t++)
	{
		...
		delta = reward - average_reward_estimate + ApproximateStateValueFunction(number_of_features, next_feature_vector, state_value_weights) - ApproximateStateValueFunction(number_of_features, feature_vector, state_value_weights);
		
		/* Update average reward estimate */
		average_reward_estimate += average_reward_estimate_alpha * delta;
		
		/* Update state-value eligibility traces */
		for (i = 0; i < number_of_features; i++)
		{
			state_value_eligibility_trace[i] = state_value_trace_decay_lambda * state_value_eligibility_trace[i] + feature_vector[i];
		} // end of i loop
		state_value_eligibility_trace[number_of_features] = state_value_trace_decay_lambda * state_value_eligibility_trace[number_of_features] + 1.0; // bias feature term
		...
		for (i = 0; i < max_number_of_actions; i++)
		{
			if (i == action_index)
			{
				...
				{
					policy_eligibility_trace[i * number_of_features + j] = policy_trace_decay_lambda * policy_eligibility_trace[i * number_of_features + j] + ((1.0 - policy[i]) * feature_vector[j]);
				} // end of j loop
			}
			else
			{
				for (j = 0; j < number_of_features; j++)
				{
					policy_eligibility_trace[i * number_of_features + j] = policy_trace_decay_lambda * policy_eligibility_trace[i * number_of_features + j] + (-policy[i] * feature_vector[j]);
				} // end of j loop				
			}
		} // end of i loop
