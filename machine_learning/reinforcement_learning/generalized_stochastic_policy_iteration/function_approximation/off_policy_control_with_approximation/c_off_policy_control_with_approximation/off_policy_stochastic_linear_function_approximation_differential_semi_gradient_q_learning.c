...
int main(int argc, char* argv[])
{
	...
	unsigned int* approximate_state_action_value_function_max_tie_stack;
	approximate_state_action_value_function_max_tie_stack = malloc(sizeof(unsigned int) * max_number_of_actions);
	for (i = 0; i < max_number_of_actions; i++)
	{
		approximate_state_action_value_function_max_tie_stack[i] = 0;
	} // end of i loop
	...
	free(policy);
	free(approximate_state_action_value_function_max_tie_stack);
...
{
	unsigned int i, j;
	unsigned int successor_state_transition_index, next_state_index, next_action_index, max_action_count;
	...
	for (i = 0; i < maximum_episode_length; i++)
	{
		...
		for (j = 0; j < max_number_of_actions; j++)
		{
			approximate_state_action_value_function[j] = ApproximateStateActionValueFunction(number_of_state_tilings, number_of_state_tiles, next_state_tile_indices, j, weights);
			
			if (max_action_value < approximate_state_action_value_function[j])
			{
				max_action_value = approximate_state_action_value_function[j];
				approximate_state_action_value_function_max_tie_stack[0] = j;
				max_action_count = 1;
			}
			else if (max_action_value == approximate_state_action_value_function[j])
			{
				approximate_state_action_value_function_max_tie_stack[max_action_count];
				max_action_count++;					
			}
		} // end of j loop
		
		next_action_index = approximate_state_action_value_function_max_tie_stack[rand() % max_action_count];
