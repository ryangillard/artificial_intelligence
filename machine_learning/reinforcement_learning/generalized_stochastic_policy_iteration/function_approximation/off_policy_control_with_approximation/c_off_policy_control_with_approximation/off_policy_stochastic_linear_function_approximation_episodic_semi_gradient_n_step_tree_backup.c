...
void GreedyPolicyFromStateActionFunction(unsigned int max_number_of_actions, unsigned int number_of_state_tilings, unsigned int number_of_state_tiles, unsigned int* state_tile_indices, double* weights, double* approximate_state_action_value_function, double* policy, double* policy_cumulative_sum);
...
unsigned int InitializeEpisode(unsigned int number_of_non_terminal_states, unsigned int max_number_of_actions, unsigned int* state_tile_indices, unsigned int number_of_state_tilings, unsigned int number_of_state_tiles, double** state_double_variables, unsigned int number_of_state_double_variables, int** state_int_variables, unsigned int number_of_state_int_variables, double* weights, double* approximate_state_action_value_function, double* policy, double* policy_cumulative_sum, unsigned int maximum_episode_length, struct Episode* episode_log)
{
	...
	/* Choose policy for chosen state by greedily choosing from the approximate state-action-value function */
	GreedyPolicyFromStateActionFunction(max_number_of_actions, number_of_state_tilings, number_of_state_tiles, state_tile_indices, weights, approximate_state_action_value_function, policy, policy_cumulative_sum);
...
void GreedyPolicyFromStateActionFunction(unsigned int max_number_of_actions, unsigned int number_of_state_tilings, unsigned int number_of_state_tiles, unsigned int* state_tile_indices, double* weights, double* approximate_state_action_value_function, double* policy, double* policy_cumulative_sum)
{
	...
	for (i = 0; i < max_number_of_actions; i++)
	{
		...
		if (fabs(approximate_state_action_value_function[i] - max_state_action_value) <= EPS)
		{
			max_action_count++;
		}
...
{
	int t, i, k, tau = 0;
	unsigned int t_mod_n_plus_1, t_plus_1_mod_n_plus_1, k_mod_n_plus_1, tau_plus_n_mod_n_plus_1, tau_mod_n_plus_1, successor_state_transition_index;
	double probability, expected_return = 0.0, approximate_state_value_function_expected_value_on_policy = 0.0;
	...
	for (t = 0; t < maximum_episode_length; t++)
	{
		...
		if (t < max_timestep)
		{
			...
			else
			{
				...
				/* Get next action */
				episode_log[t_plus_1_mod_n_plus_1].action_index = rand() % max_number_of_actions; // randomly choose a next action as a function of the next state
			}
		}
		...
		if (tau >= 0)
		{
			...
			else
			{
				...
				approximate_state_value_function_expected_value_on_policy = 0.0;
				
				for (i = 0; i < max_number_of_actions; i++)
				{
					/* Add to expected value */
					approximate_state_value_function_expected_value_on_policy += policy[i] * ApproximateStateActionValueFunction(number_of_state_tilings, number_of_state_tiles, state_tile_indices, i, weights);
				} // end of j loop
				
				/* Calculate expected return */
				expected_return = episode_log[t_plus_1_mod_n_plus_1].reward + discounting_factor_gamma * approximate_state_value_function_expected_value_on_policy;
			}
			...
			for (k = min(t, max_timestep - 1); k >= tau + 1; k--)
			{
				...
				/* Get tiled feature indices of next state */
				GetTileIndices(number_of_state_tilings, number_of_state_tiles, state_double_variables[episode_log[k_mod_n_plus_1].state_index], number_of_state_double_variables, state_int_variables[episode_log[k_mod_n_plus_1].state_index], number_of_state_int_variables, state_tile_indices);
				...
				/* Calculate expected return */
				expected_return = episode_log[k_mod_n_plus_1].reward + discounting_factor_gamma * (approximate_state_value_function_expected_value_on_policy + policy[episode_log[k_mod_n_plus_1].action_index] * expected_return);
			} // end of k loop
