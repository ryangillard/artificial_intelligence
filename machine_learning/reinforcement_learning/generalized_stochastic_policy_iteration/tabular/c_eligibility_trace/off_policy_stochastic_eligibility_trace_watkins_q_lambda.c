...
void UpdateEligibilityTraceAndStateActionValueFunction(unsigned int state_index, unsigned int action_index, unsigned int next_action_index, unsigned int max_action_index, double delta, unsigned int number_of_non_terminal_states, unsigned int* number_of_actions_per_non_terminal_state, double alpha, double discounting_factor_gamma, double trace_decay_lambda, unsigned int trace_update_type, double** state_action_value_function, double** eligibility_trace);
...
int main(int argc, char* argv[])
{
	...
	for (i = 0; i < number_of_episodes; i++)
	{
		...
		InitializeEpisode(number_of_non_terminal_states, number_of_actions_per_non_terminal_state, state_action_value_function, epsilon, policy, policy_cumulative_sum, eligibility_trace, &initial_state_index, &initial_action_index);
...
{
	unsigned int i, j;
	unsigned int successor_state_transition_index, next_state_index, next_action_index, max_action_count, max_action_index;
	double probability, reward, delta, max_action_value;
	...
	for (i = 0; i < maximum_episode_length; i++)
	{
		...
		if (next_state_index >= number_of_non_terminal_states)
		{
			...
			UpdateEligibilityTraceAndStateActionValueFunction(state_index, action_index, 0, 0, delta, number_of_non_terminal_states, number_of_actions_per_non_terminal_state, alpha, discounting_factor_gamma, trace_decay_lambda, trace_update_type, state_action_value_function, eligibility_trace);
		...
		else
		{
			...
			{
				max_action_index = state_action_value_function_max_tie_stack[next_state_index][rand() % max_action_count];
			}
			...
			/* Update eligibility traces and state action value function with TD error */
			UpdateEligibilityTraceAndStateActionValueFunction(state_index, action_index, next_action_index, max_action_index, delta, number_of_non_terminal_states, number_of_actions_per_non_terminal_state, alpha, discounting_factor_gamma, trace_decay_lambda, trace_update_type, state_action_value_function, eligibility_trace);
