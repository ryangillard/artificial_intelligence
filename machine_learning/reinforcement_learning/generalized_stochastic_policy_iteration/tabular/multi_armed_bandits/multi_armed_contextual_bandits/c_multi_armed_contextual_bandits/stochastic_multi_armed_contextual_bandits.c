...
int main(int argc, char* argv[])
{
	...
	double** bandit_mean;
	bandit_mean = malloc(sizeof(double*) * number_of_states);
	for (i = 0; i < number_of_states; i++)
	{
		bandit_mean[i] = malloc(sizeof(double) * number_of_bandits);
		for (j = 0; j < number_of_bandits; j++)
		{
			bandit_mean[i][j] = RNorm(global_bandit_mean_mean, sqrt(global_bandit_mean_variance));
		} // end of j loop
	} // end of i loop
	...
	bandit_variance = malloc(sizeof(double*) * number_of_states);
	for (i = 0; i < number_of_states; i++)
	{
		bandit_variance[i] = malloc(sizeof(double) * number_of_bandits);
		for (j = 0; j < number_of_bandits; j++)
		{
			bandit_variance[i][j] = RNorm(global_bandit_variance_mean, sqrt(global_bandit_variance_variance));
		} // end of j loop
	} // end of i loop
	...
	for (i = 0; i < number_of_states; i++)
	{
		state_action_value_function[i] = malloc(sizeof(double) * number_of_bandits);
		for (j = 0; j < number_of_bandits; j++)
		{
	...
	action_count = malloc(sizeof(unsigned int*) * number_of_states);
	for (i = 0; i < number_of_states; i++)
	{
		action_count[i] = malloc(sizeof(unsigned int) * number_of_bandits);
		for (j = 0; j < number_of_bandits; j++)
		{
			action_count[i][j] = 0.0;
		} // end of j loop
	} // end of i loop
	
	/* Create action trace array */
	double** action_trace;
	action_trace = malloc(sizeof(double*) * number_of_states);
	for (i = 0; i < number_of_states; i++)
	{
		action_trace[i] = malloc(sizeof(double) * number_of_bandits);
		for (j = 0; j < number_of_bandits; j++)
		{
			action_trace[i][j] = 0.0;
		} // end of j loop
	} // end of i loop
	
	/* Create policy array */
	double** policy;
	policy = malloc(sizeof(double*) * number_of_states);
	for (i = 0; i < number_of_states; i++)
	{
		policy[i] = malloc(sizeof(double) * number_of_bandits);
		for (j = 0; j < number_of_bandits; j++)
		{
			policy[i][j] = 1.0 / number_of_bandits;
		} // end of j loop
	} // end of i loop
	
	/* Create policy cumulative sum array */
	double** policy_cumulative_sum;
	policy_cumulative_sum = malloc(sizeof(double*) * number_of_states);
	for (i = 0; i < number_of_states; i++)
	{
		policy_cumulative_sum[i] = malloc(sizeof(double) * number_of_bandits);
		policy_cumulative_sum[i][0] = policy[i][0];
		for (j = 1; j < number_of_bandits; j++)
		{
	...
	printf("state\tbandit\tmean\tvar\tchg_frq\n");
	for (i = 0; i < number_of_states; i++)
	{
		for (j = 0; j < number_of_bandits; j++)
		{
			printf("%u\t%u\t%lf\t%lf\t%u\n", i, j, bandit_mean[i][j], bandit_variance[i][j], bandit_stochastic_change_frequencies[i]);
		} // end of j loop
	} // end of i loop
...
void LoopThroughIterations(unsigned int number_of_iterations, unsigned int number_of_states, unsigned int number_of_bandits, double** bandit_mean, double** bandit_variance, unsigned int* bandit_stochastic_change_frequencies, unsigned int* bandit_stochastic_change_counter, double global_bandit_mean_mean, double global_bandit_mean_variance, double global_bandit_variance_mean, double global_bandit_variance_variance, double** state_action_value_function, unsigned int** action_count, double** action_trace, double** policy, double** policy_cumulative_sum, double alpha, double epsilon, int action_selection_type, int action_value_update_type)
{
	...
	for (t = 0; t < number_of_iterations; t++)
	{
		...
		reward = RNorm(bandit_mean[state_index][action_index], bandit_variance[state_index][action_index]);
		...
		for (i = 0; i < number_of_bandits; i++)
		{
			if (bandit_stochastic_change_frequencies[i] > 0)
			{
				...
				if (bandit_stochastic_change_counter[i] == bandit_stochastic_change_frequencies[i])
				{
					...
					{
						bandit_mean[j][i] = RNorm(global_bandit_mean_mean, sqrt(global_bandit_mean_variance));
						bandit_variance[j][i] = RNorm(global_bandit_variance_mean, sqrt(global_bandit_variance_variance));
...
void UpdatePolicyFromStateActionValueFunction(unsigned int state_index, unsigned int number_of_bandits, double** state_action_value_function, unsigned int** action_count, unsigned int iteration_count, double epsilon, int action_selection_type, double** policy, double** policy_cumulative_sum)
{
	...
	for (i = 0; i < number_of_bandits; i++)
	{
		...
		{
			action_value = state_action_value_function[state_index][i];
		}
		else if (action_selection_type == 2) // upper-confidence-bound
		{
			if (action_count[state_index][i] == 0)
			{
			...
			{
				action_value = state_action_value_function[state_index][i] + epsilon * sqrt(log((double)iteration_count) / action_count[state_index][i]);
			}
		}
