...
int main(int argc, char* argv[])
{
	...
	/* Track the how long it has been for each state-action pair was last visited from the environment */
	unsigned int** model_state_action_time_since_last_visit;
	
	model_state_action_time_since_last_visit = malloc(sizeof(unsigned int*) * number_of_non_terminal_states);
	for (i = 0; i < number_of_non_terminal_states; i++)
	{
		model_state_action_time_since_last_visit[i] = malloc(sizeof(unsigned int) * number_of_actions_per_non_terminal_state[i]);
		for (j = 0; j < number_of_actions_per_non_terminal_state[i]; j++)
		{
			model_state_action_time_since_last_visit[i][j] = 0;
		} // end of j loop
	} // end of i loop
	...
	for (i = 0; i < number_of_non_terminal_states; i++)
	{
		...
		free(model_state_action_time_since_last_visit[i]);
	...
	free(model_state_action_time_since_last_visit);
...
{	
	...
	if ((*model_number_of_seen_non_terminal_states) == 0 || (model_seen_non_terminal_states_stack_reverse_lookup[state_index] == 0 && model_seen_non_terminal_states_stack[0] != state_index)) // if new state
	{
		...
		model_state_action_time_since_last_visit[state_index][model_number_of_seen_non_terminal_states_actions[state_index]] = 0;
	...
	else // if already visited state
	{
		...
		{
			model_state_action_time_since_last_visit[state_index][model_seen_non_terminal_states_actions_stack_reverse_lookup[state_index][action_index]]++;
		}
	}
...
void ModelSimualatePlanning(unsigned int number_of_planning_steps, unsigned int number_of_non_terminal_states, unsigned int* number_of_actions_per_non_terminal_state, unsigned int model_number_of_seen_non_terminal_states, unsigned int* model_seen_non_terminal_states_stack, unsigned int* model_seen_non_terminal_states_stack_reverse_lookup, unsigned int* model_number_of_seen_non_terminal_states_actions, unsigned int** model_seen_non_terminal_states_actions_stack, unsigned int** model_seen_non_terminal_states_actions_stack_reverse_lookup, unsigned int** model_state_action_time_since_last_visit, unsigned int** model_number_of_state_action_successor_states, unsigned int*** model_state_action_successor_state_indices, double*** model_state_action_successor_state_transition_probabilities, double*** model_state_action_successor_state_transition_probabilities_cumulative_sum, double*** model_state_action_successor_state_rewards, unsigned int*** model_state_action_successor_state_number_of_visits, unsigned int** state_action_value_function_max_tie_stack, double alpha, double discounting_factor_gamma, double kappa, double** state_action_value_function)
{
	...
	for (i = 0; i < number_of_planning_steps; i++)
	{
		...
		/* Get reward from state and action */
		reward = model_state_action_successor_state_rewards[state_index][action_index][successor_state_transition_index] + kappa * sqrt(model_state_action_time_since_last_visit[state_index][model_seen_non_terminal_states_actions_stack_reverse_lookup[state_index][action_index]]);
