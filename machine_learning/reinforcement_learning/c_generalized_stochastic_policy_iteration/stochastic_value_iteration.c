...
int main(int argc, char* argv[])
{
	...
	/* Set the maximum number of value iterations */
	unsigned int maximum_number_of_value_iterations = 20;
	
	/*********************************************************************************************************/
	/****************************************** RUN VALUE ITERATION ******************************************/
	/*********************************************************************************************************/
		
	printf("Initial state value function:\n");
	for (i = 0; i < number_of_states; i++)
	{
		printf("%lf\n", state_value_function[i]);
	} // end of i loop
	...
	/* Value iteration */
	ValueIteration(number_of_non_terminal_states, number_of_actions_per_non_terminal_state, number_of_state_action_successor_states, state_action_successor_state_indices, state_action_successor_state_transition_probabilities, state_action_successor_state_rewards, convergence_threshold, discounting_factor_gamma, maximum_number_of_value_iterations, state_value_function, state_action_value_function);
	
	/* Greedy Policy Selection */
	GreedyPolicySelection(number_of_non_terminal_states, number_of_actions_per_non_terminal_state, number_of_state_action_successor_states, state_action_successor_state_indices, state_action_successor_state_transition_probabilities, state_action_successor_state_rewards, policy, discounting_factor_gamma, state_value_function);
...
{
	unsigned int i, j, k;
	double delta = 0.0, temp_state_value_function = 0.0, temp_state_action_value_function = 0.0, max_state_action_value = -DBL_MAX;
	unsigned int number_of_value_iterations = 0;
	...
	do
	{
		...
		for (i = 0; i < number_of_non_terminal_states; i++)
		{
			...
			for (j = 0; j < number_of_actions_per_non_terminal_state[i]; j++)
			{
				...
				if (state_action_value_function[i][j] > max_state_action_value)
				{
					max_state_action_value = state_action_value_function[i][j];
				}
			} // end of j loop
			
			state_value_function[i] = max_state_action_value;
		...
		number_of_value_iterations++;
	} while (delta >= convergence_threshold && number_of_value_iterations < maximum_number_of_value_iterations);
	
	return;
} // end of ValueIteration function
...
} // end of GreedyPolicySelection function