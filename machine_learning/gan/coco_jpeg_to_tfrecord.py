...
def _convert_to_example(image_buffer, bb_x, bb_y, bb_width, bb_height):
  ...
  Args:
    ...
    bb_x: int, upper left x coord of BB.
    bb_y: int, upper left y coord of BB.
    bb_width: int, width of BB.
    bb_height: int, height of BB.
  Returns:
    Example proto
  """
  colorspace = 'RGB'
  ...
  example = tf.train.Example(
      ...
      features=tf.train.Features(
          feature={
              'bb_ymin': _int64_feature(bb_y)
          }
      ),
      features=tf.train.Features(
          feature={
              'bb_xmin': _int64_feature(bb_x)
          }
      ),
      features=tf.train.Features(
          feature={
              'bb_ymax': _int64_feature(bb_y + bb_height)
          }
      ),
      features=tf.train.Features(
          feature={
              'bb_xmax': _int64_feature(bb_x + bb_width)
          }
      )
  )
  return example
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      ...
      help=
      'Path to input.  Each CSV line of input has 5 fields image-file-name, bb_x, bb_y, bb_width, bb_height',
      required=True)
  parser.add_argument(
      '--validation_csv',
      # pylint: disable=line-too-long
